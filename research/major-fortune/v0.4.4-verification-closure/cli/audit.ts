...
import { runTemporalIndependenceAudit } from "../temporal/run-temporal-audit.js";
import { runTelemetryAudit } from "../telemetry/audit-events.js";
...
function run(): void {
  ...
  const timelinePointsFallback = runTimelineMode("nam-phai", false);
  writeFileSync(join(RAW_DIR, "timeline-points-nam-phai-fallback.json"), JSON.stringify(timelinePointsFallback, null, 2), "utf8");
  console.log(`[audit] Timeline points (Fallback): ${timelinePointsFallback.length}`);

  console.log("[audit] Running timeline observations (Nam Phái, flag=ON)...");
  const timelinePointsEnabled = runTimelineMode("nam-phai", true);
  writeFileSync(join(RAW_DIR, "timeline-points-nam-phai-enabled.json"), JSON.stringify(timelinePointsEnabled, null, 2), "utf8");
  console.log(`[audit] Timeline points (Enabled): ${timelinePointsEnabled.length}`);

  console.log("[audit] Running timeline observations (Trung Châu, flag=OFF)...");
  const timelinePointsControl = runTimelineMode("trung-chau", false);
  writeFileSync(join(RAW_DIR, "timeline-points-trung-chau-control.json"), JSON.stringify(timelinePointsControl, null, 2), "utf8");
  console.log(`[audit] Timeline points (Control): ${timelinePointsControl.length}`);
  ...
  console.log(`[audit] Temporal pairs tested: ${temporalReport.totalAttemptedPairs}, Passed: ${temporalReport.passed}`);
