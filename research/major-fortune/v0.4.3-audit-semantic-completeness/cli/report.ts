...
import { evaluateTimelineEquivalence } from "../timeline/build-timeline-pairs.js";
...
function run(): void {
  ...
  let currentChartFb = "";
  let cycleFb = 0;
  for (const o of namPhaiBaseline) {
    if (o.chartFixtureId !== currentChartFb) {
      currentChartFb = o.chartFixtureId;
      cycleFb = 0;
    }
    o.cycleOrder = cycleFb++;
    o.observationId = buildObservationId(o.corpusId, o.school, o.chartFixtureId, o.cycleIndex, o.activePalaceIndex);
  }

  let currentChartTc = "";
  let cycleTc = 0;
  for (const o of trungChauBaseline) {
    if (o.chartFixtureId !== currentChartTc) {
      currentChartTc = o.chartFixtureId;
      cycleTc = 0;
    }
    o.cycleOrder = cycleTc++;
    o.observationId = buildObservationId(o.corpusId, o.school, o.chartFixtureId, o.cycleIndex, o.activePalaceIndex);
  }
  ...
  const timelineEquiv = evaluateTimelineEquivalence(fallback, timelinePoints);
  writeFileSync(join(REPORTS_DIR, "timeline-equivalence-report.json"), JSON.stringify(timelineEquiv, null, 2), "utf8");
  console.log(`[report] Timeline equivalence: passed=${timelineEquiv.passed}, mismatches=${timelineEquiv.mismatchingObservationCount}, missing current=${timelineEquiv.missingCurrentIds.length}`);

  if (timelineEquiv.missingCurrentIds.length > 0) {
    const fallbackMap = new Map(fallback.map((o) => [o.observationId, o]));
    const classifications = classifyUnmatchedTimelinePoints(timelineEquiv.missingCurrentIds, fallbackMap);
    writeFileSync(join(REPORTS_DIR, "timeline-unmatched-classification-report.json"), JSON.stringify(classifications, null, 2), "utf8");
  }
