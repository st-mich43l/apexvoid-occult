/**
 * Major Fortune V0.4.3 Determinism Check.
 *
...
const RUN1_DIR = process.env["MF_V042_RUN1_DIR"] ?? "/tmp/mf-v043-run-1";
...
function run(): void {
  ...
  if (!existsSync(RUN1_DIR)) {
    ...
    console.error("[determinism] Run the audit twice and set MF_V042_RUN1_DIR or use /tmp/mf-v043-run-1");
    process.exit(1);
  }
