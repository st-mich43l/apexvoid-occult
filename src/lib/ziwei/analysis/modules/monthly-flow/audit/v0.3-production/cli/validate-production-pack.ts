/**
 * Validate Monthly Flow V0.3.0 production production research pack presence + routing.
 */
...
function main(): void {
  ...
  const required = [
    "README.md",
    "V0.3.0-PRODUCTION-HARDENING-DECISION.md",
  ...
  if (routing.status !== "available" || routing.version !== "0.3.0") {
  ...
  if (existsSync(join(pack, "reports/decision.json"))) {
    ...
    const md = readFileSync(join(pack, "V0.3.0-PRODUCTION-HARDENING-DECISION.md"), "utf8");
