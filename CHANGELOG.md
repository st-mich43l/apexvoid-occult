...
### Thêm Mới (Added)
- **Tử Vi**: Profile chấm điểm Lưu Nguyệt experimental `nam-phai-monthly-v2` (opt-in) — multi-axis benefit/risk/activation/conflict/stability/confidence, soft saturation, dominance chính tinh, mượn đối cung VCD, cộng hưởng Đại vận linked và pattern declarative; mặc định production vẫn `legacy-v1`. Preferred ranges là soft calibration bands (không fail CI); expose subtotal buckets.
